...
export default function AnimeDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch episodes
        const episodeRes = await fetch(`https://www.sankavollerei.com/anime/episode/${slug}`);
        if (episodeRes.ok) {
        ...
        setBookmark(bookmarks.some(b => b.id === slug && b.type === 'anime'));
      } catch (error) {
  ...
  const toggleBookmark = () => {
    ...
    if (bookmark) {
      // Remove bookmark
      const newBookmarks = bookmarks.filter(b => !(b.id === slug && b.type === 'anime'));
  ...
  const watchEpisode = (episode) => {
    ...
    const filteredHistory = history.filter(h => !(h.id === slug && h.type === 'anime' && h.episode === episode.title));
  ...
  if (!anime) {
    return (
      <div className="container mx-auto px-4 py-6 text-center">
        ...
        <Link href="/anime">
          <a className="text-indigo-400 hover:text-indigo-300">Kembali ke daftar anime</a>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <Head>
        <title>{anime.title} - Anime - LNNZEPHRY</title>
        <meta name="description" content={`Nonton ${anime.title} subtitle Indonesia`} />
      </Head>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-4xl">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{anime.title} - {currentEpisode.title}</h2>
              <button 
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row gap-6 mb-8">
              ...
              <div className="p-4">
                <div className="flex justify-between items-center mb-4">
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                    anime.status === 'Ongoing' ? 'bg-green-900 text-green-300' : 'bg-blue-900 text-blue-300'
                  }`}>
                    {anime.status}
                  </span>
                  <button 
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="ml-1">{anime.rating || 'N/A'}</span>
                    </div>
                  </div>
          ...
          <div className="w-full md:w-2/3">
            ...
            <p className="text-gray-400 mb-6">{anime.japanese_title}</p>
